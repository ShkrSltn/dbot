...
from .prompt_management import display_prompt_management
import json

def display_prompt_engineer(sample_statements):
    st.title("🛠️ Prompt Engineer")
    ...
    if not st.session_state.prompts or len(st.session_state.prompts) <= 4:  # Only default prompts exist
        user_prompts = get_user_prompts(st.session_state.user["id"])
        if user_prompts:
            st.session_state.prompts.update(user_prompts)
    ...
    tab1, tab2, tab3 = st.tabs(["🛠️ Prompt Engineer", "📝 Manage Prompts", "📊 History"])
    ...
    with tab1:
        ...
        with col1:
            ...
            if prompt_option == "Use Existing Prompt":
                ...
                if selected_prompt not in ['default', 'basic', 'digcomp_few_shot', 'general_few_shot']:
                    st.markdown("---")
